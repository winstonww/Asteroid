...
fun parseAsteroidsJsonResult(jsonResult: JSONObject): ArrayList<NetworkAsteroid> {
    ...
    for (formattedDate in nextSevenDaysFormattedDates) {
        Log.i("parseAsteroidsJsonResult", nextSevenDaysFormattedDates.toString())
        try {
            nearEarthObjectsJson.keys().forEach {
                Log.i("parseAsteroidsJsonResult", "key: $it")
            }
            ...
            for (i in 0 until dateAsteroidJsonArray.length()) {
                ...
                Log.i("parseAsteroidsJsonResult", asteroid.toString())
                asteroidList.add(asteroid)
            }
        } catch (e: JSONException) {
            Log.i("parseAsteroidsJsonResult", e.toString())
        }
